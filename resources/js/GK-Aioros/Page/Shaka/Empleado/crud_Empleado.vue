<template>

  <div class="text-center">
    <v-dialog v-model="dialog" width="100%" >
      <template v-slot:activator="{ on, attrs }" >
        <v-btn
          class="mx-2 " 
          :title="tipo" 
          fab  
          outlined 
          x-small 
          v-bind="attrs"
          v-on="on"
          :color="(tipo == 'Nuevo')?'green':'cyan'"
         
        >
            <v-icon v-if="(tipo == 'Modificando')" dark outlined> mdi-pencil </v-icon>
            <v-icon v-else outlined>mdi-plus</v-icon>
          
        </v-btn>
            
      </template>

      <v-card>
        <v-card-title class="blue lighten-1 text-white"> Empleado </v-card-title>

        <v-card-text>
          <template>
            <v-form v-model="valid">
              <v-container>
                <v-row>    
                    <v-col cols="12" md="2">
                      <v-avatar width="170px" height="170px">
                        <img
                          src="https://cdn.vuetifyjs.com/images/john.jpg"
                          alt="John"
                        >
                      </v-avatar>
                    </v-col>
                    <v-col cols="12" md="8">
                      <v-row>
                        <v-col cols="12" md="4">
                            <v-text-field
                                v-model="vempleado.nombreApellido"
                                label="Nombre y Apellido"
                                clearable
                            ></v-text-field>
                        </v-col>
                        <v-col cols="12" md="4">
                            <v-text-field
                                v-model="vempleado.dni"
                                label="DNI"
                                clearable
                            ></v-text-field>
                                        
                        </v-col>
                        <v-col cols="12" md="4">
                            <v-text-field
                                v-model="vempleado.domicilio"
                                label="Domicilio"
                                clearable
                            ></v-text-field>
                                        
                        </v-col>
                      </v-row>
                      <v-row>
                        <v-col cols="12" md="4">
                            <v-text-field
                                v-model="vempleado.telefono"
                                label="Teléfono"
                                clearable
                            ></v-text-field>
                                        
                        </v-col>
                        <v-col cols="12" md="4">
                            <v-text-field
                                v-model="vempleado.observacion"
                                label="Observación"
                                clearable
                            ></v-text-field>
                                        
                        </v-col>
                        <v-col cols="12" md="4">
                            <v-text-field
                                v-model="vempleado.nrolegajo"
                                label="N° de Legajo"
                                clearable
                            ></v-text-field>
                                        
                        </v-col>
                        
                      </v-row>
                    </v-col>
                    <v-col cols="12" md="2">
                       <QRCode :text="vempleado.dni" :size="140"></QRCode>
                    </v-col>
                    <v-col cols="12" md="12">
                      <v-row>
                        
                        <v-col  cols="12" md="3">Personal Temporal
                            <v-container
                                class="px-0"
                                fluid
                            >
                                <v-switch
                                v-model="vempleado.temporal"
                                :label="(vempleado.temporal == 1)?'SI':'NO'"
                                ></v-switch>
                            </v-container>
                        </v-col>
                        <v-col  cols="12" md="3"> Estado
                            <v-container
                                class="px-0"
                                fluid
                            >
                                <v-switch
                                v-model="vempleado.estado"
                                :label="(vempleado.estado == 1)?'Activo':'Inactivo'"
                                ></v-switch>
                            </v-container>
                        </v-col>
                        <v-col cols="12" md="3">
                          <v-menu
                            v-model="menu2"
                            :close-on-content-click="false"
                            :nudge-right="40"
                            transition="scale-transition"
                            offset-y
                            min-width="auto"
                          >
                            <template v-slot:activator="{ on, attrs }">
                              <v-text-field
                                v-model="date"
                                label="Fecha de Ingreso"
                                prepend-icon="mdi-calendar"
                                readonly
                                v-bind="attrs"
                                v-on="on"
                              ></v-text-field>
                            </template>
                            <v-date-picker
                              v-model="date"
                              @input="menu2 = false"
                            ></v-date-picker>
                          </v-menu>
                        </v-col>
                      </v-row>
                    </v-col>
                </v-row>
                <v-divider></v-divider>
                <v-row>
                  <v-col cols="12" md="12">
                    <VincualacionPT :object="object" :title="'Vinculación de Puestos y Tareas'"/>
                  </v-col>
                </v-row>
                <v-divider></v-divider>
                               
              </v-container>
            </v-form>
          </template>
        </v-card-text>

        <v-divider></v-divider>

        <v-card-actions>
          <v-spacer></v-spacer>
          <msjAlerta :msj="alerta"></msjAlerta>
          <v-btn color="primary" text @click="closealert()"> Salir </v-btn>
          <v-btn v-if="(tipo == 'Nuevo')" color="primary" text @click="nuevo()"> Guardar </v-btn>
          <v-btn v-else color="primary" text @click="modificar()"> Modificar </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <loader :load="load"></loader>
  </div>
</template>
<script>
import VincualacionPT from './Vinculacion/VinculacionPT.vue'
import QRCode from 'vue-qrcode-component'

export default {
  props: ['empleado','tipo'],
  data() {
    return {
     
      alerta: { mensaje: "", visible: false, tipo: 0 },
      load:false,
      valid:true,
      dialog:false,
     
      vempleado:{
              nombreApellido : '', 
              dni : '', 
              domicilio: '', 
              telefono: '', 
              observacion :'',
              nrolegajo:'',
              fechaingreso:'',
              qr:'',
              foto :'', 
              temporal: '', 
              estado: '', 
              user_id: '', 
              empresa_id: ''
              
          },
      date: (new Date(Date.now() - (new Date()).getTimezoneOffset() * 60000)).toISOString().substr(0, 10),
      menu2: false,
      object:[],
      
    }
  },
  watch:{
    empleado(s){
      console.log('nuevo_empleado',s)
      this.vempleado = s.data
      this.dialog = s.visible
    }
  },
  components:{
    QRCode,
    VincualacionPT
  },
  methods: {
      
        nuevo(){
            let formData = new FormData();
            formData.append('nombre', this.vempleado.nombre)
            formData.append('descripcion', this.vempleado.descripcion)
            formData.append('estado', this.vempleado.estado)
            
            this.load=true
            try {
                axios
                .post(this.empleado.page, formData)
                .then((response) => {
                    
                    if (response.data.status == 0) {
                        //alerta//
                        this.alerta.mensaje = response.data.descripcion;
                        this.alerta.tipo = 2;
                        this.alerta.color = 'success';
                        this.alerta.visible = true;
                        this.refresh();
                    } else {
                        if (response.data.descripcion != ""){
                            this.alerta.mensaje = response.data.descripcion;
                        }else{
                            this.alerta.mensaje = funciones.errorjson(response.data.errors)    
                        }
                            this.alerta.tipo = 2;
                            this.alerta.color = 'error';
                            this.alerta.visible = true;
                        
                    }
                })
                .catch((error) => {
                    console.log(error);
                    this.load=false
                })
                .finally(() => this.load=false);
            } catch (error) {
                console.log(error);
            }
      
        },
        closealert() {
           
            this.dialog = false;
            this.alerta = { mensaje: "", visible: false, tipo: 0 };
            
        },
  },
}
</script>