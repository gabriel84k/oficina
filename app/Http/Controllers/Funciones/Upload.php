    ...
    function getsize($size) {
        try{
            if (is_numeric($size) || $size===null){
                ...
                switch($last) {
                    case 'g':
                        $size = str_replace('g','',strtolower($size));
                        $size *= 1024;
                    case 'm':
                        $size = str_replace('m','',strtolower($size));
                        $size *= 1024;
                    
                    case 'k':
                        $size = str_replace('k','',strtolower($size));
                        $size *= 1024;
                }
            }
            return $size;
        }catch(\Throwable $th){
            dd('descripcion: '.$th->getMessage().' Linea: '.$th->getLine().' size: '.$size);
        }
    }
